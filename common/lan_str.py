...
class LanStr:
    ...
    AUTO_LOOP = "Auto Loop"
    AUTO_LOOP_INTERVAL = "Interval (min)"
    AUTO_LOOP_TIP = ("When the session timer ends and the game finishes, the browser is closed. "
                     "With Auto Loop on, the browser relaunches and auto-join restarts after the "
                     "interval (minutes), with the session timer reset to its set duration.")
    SESSION_MIN = "Session (min)"
    START = "Start"
    STOP = "Stop"
    AUTO_STATUS_IDLE = "Idle"
    AUTO_STATUS_PLAYING = "Auto-joining, ends in"
    AUTO_STATUS_ENDING = "Closing browser after this game"
    AUTO_STATUS_LOOPING = "Relaunch in"
...
class LanStrZHS(LanStr):
    ...
    AUTO_LOOP = "自动循环"
    AUTO_LOOP_INTERVAL = "间隔(分)"
    AUTO_LOOP_TIP = ("时长结束且本局游戏结束后，浏览器会关闭。开启自动循环后，"
                     "将在设置的间隔(分钟)后重新打开浏览器并重启自动加入，时长会重置为设置值。")
    SESSION_MIN = "时长(分)"
    START = "开始"
    STOP = "停止"
    AUTO_STATUS_IDLE = "空闲"
    AUTO_STATUS_PLAYING = "自动加入中，剩余"
    AUTO_STATUS_ENDING = "本局结束后关闭浏览器"
    AUTO_STATUS_LOOPING = "重新打开倒计时"
