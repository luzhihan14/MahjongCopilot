...
```bash
# One-shot dev setup (macOS/Linux): venv/conda + deps + Chromium + placeholder models + smoke test
./setup.sh                          # FORCE_MODELS=1 re-downloads models; PYTHON_BIN=... picks the interpreter

# Manual setup (Python 3.11 recommended) — note: does NOT fetch models; setup.sh does
python -m venv venv
...
# Local model type needs weights in models/ (mortal.pth 4P, mortal_3p.pth 3P); 3P also needs libriichi3p/*.so|*.pyd
...
# Retrain the popup-icon detector after adding/relabeling snapshots in training_data/popups/
# (writes resources/icon_detector.pth; prints an eval that must end "ALL PASS")
venv/bin/python scripts/train_icon_detector.py
...
- **Popup dismissal** ([game/icon_detect.py](game/icon_detect.py) + `Automation._dismiss_popup_steps`/`_join_game_iter`): while auto-join waits for the main menu, if it stays hidden `popup_dismiss_delay` seconds (a popup — announcement/event/rankings — is covering it), a tiny FCN (~90k params, torch; weights `resources/icon_detector.pth`, so they ship with the app) finds `close_x`/`return_arrow` icons and automation clicks the best match. Dismissal runs whenever the join wait-loop can't see the menu (login popups, an accidentally opened shop/event page) **except while `_queueing`** — the flag is set when the join clicks complete (trailing code of `_join_game_iter`, i.e. we just entered the match queue, where the menu template doesn't match and a false-positive click could cancel the queue) and cleared on `on_enter_game`/`on_end_game`/`on_lobby_login`/`on_exit_lobby` or any menu sighting; it goes stale after `QUEUEING_STALE_SEC` (a "queue" pending that long with no game and no menu means a missed click or a dead queue, and suppression would otherwise gate off every recovery path forever). Toggling auto-join off exits the wait loop entirely — that's the way to browse the lobby manually without dismissal acting. Detection runs on the screenshot resized to a 960×540 reference (absorbs viewport size + browser zoom), multi-scale heatmaps → peaks → 16×9 click units. Menu-check and icon detection decide on the *same frame* (the issue-#101 double-check pattern). Escalation: after `POPUP_DISMISS_MAX_ATTEMPTS` attempts (valid menu-less frames, icon found or not), one `ActionStepReload` (routed through `run_step` so the stop event gates it; the reload ends the WS flow → kills the task → a fresh login restarts a new JOIN task) — rate-limited by the **instance-level** `_popup_reload_at` cooldown (`POPUP_RELOAD_COOLDOWN_SEC`), because any per-task "reload once" flag is wiped by the very task-restart the reload causes; past the cooldown it waits passively. Missing/corrupt weights degrade gracefully (detector disabled, reload escalation still works). Training: `scripts/train_icon_detector.py` over [training_data/popups/](training_data/popups/) — `labels.json` holds the canvas region + canvas-relative icon boxes; raw screenshots and icon crops are committed (mask account info before adding screenshots); training and inference share the geometry contract via constants imported from `icon_detect`. `popup_dismiss_threshold` is a JSON-only expert knob (like `auto_retry_interval`).
