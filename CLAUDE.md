...
- **Auto-close / auto-loop** ([gui/main_gui.py](gui/main_gui.py) `_update_auto_loop`): the GUI update tick (main thread, ~50ms) runs a small state machine. When the Auto Join `Timer` expires it stops auto-join and, once `is_in_game()` has stayed False for a grace period (debounced against matchmaking/disconnect gaps), closes the browser via `BotManager.close_browser()`. If `enable_auto_loop` is set, it then waits for the browser thread to fully stop, waits `auto_loop_interval` minutes, and relaunches the browser + re-enables auto-join + restarts the timer at its captured duration (`Timer.restart_saved()`). Note `GameBrowser.stop(False)` keeps the thread ref so `is_running()` stays truthful during teardown (a relaunch must not race the dying thread).
